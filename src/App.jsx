...
async function parseMsg(text,provider,hint){
...
function OsobeBox(props){
  ...
  return React.createElement("div",{style:box},naslov,
    ...
    // Marko 15.9.: radnice nisu znale da je OVO konacna rec o datumima dece - AI je
    // posle ovoga vise ne dira, pa ispravka ovde resava "datumi za decu nisu dobri".
    React.createElement("div",{style:{color:"var(--mt)",marginBottom:"6px",lineHeight:1.5}},"Ispravi datum ovde ako nije tačan — ovo je konačno, analiza ide po ovome."),
...
export default function App(){
  ...
  // PARSE
  // OCITAJ PONOVO (Marko 15.9. "ponekad datumi za decu nisu dobri"): kad radnica
  // vidi gresku, drugo citanje NE sme da bude isti poziv jos jednom - AI dobija
  // izricit zadatak da trazi sopstvenu gresku, pre svega kod dece.
  var PONOVO_HINT="\n\n*** DRUGO CITANJE - PRETHODNO JE BILO POGRESNO ***\n"+
    "Radnica je pogledala rezultat prvog citanja i videla gresku, najcesce kod DECE: pogresan datum, ili datum vezan za pogresnu osobu. "+
    "Citas ISTU poruku ponovo, sa zadatkom da nadjes sopstvenu gresku.\n"+
    "Pre nego sto vratis JSON uradi ovo:\n"+
    "1. Izlistaj SVAKU osobu koju poruka pominje i datum koji joj u tekstu stoji NAJBLIZE (skoro uvek odmah posle imena).\n"+
    "2. Proveri da nijedno dete nije dobilo datum drugog deteta, roditelja ili partnera.\n"+
    "3. Proveri da nijedan datum nije izmisljen - ako osoba u poruci NEMA datum, ne dodeljuj joj nijedan.\n"+
    "4. Procitaj cifre svakog datuma jos jednom, cifru po cifru, iz originalne poruke.\n"+
    "Budi konzervativan: bolje ostaviti prazno nego pogoditi.";

  async function doParse(idx,provider,textOverride,ponovo){
    var s=slots[idx];
    ...
    try{
      var p=await parseMsg(pasteText,provider,ponovo?PONOVO_HINT:"");
      if(p&&p.__error){
        if(p.__overload){
        ...
        // rawPaste: kod DRUGOG citanja je s.paste vec prazan - bez "||" bi se
        // originalna poruka izgubila i "Ocitaj ponovo" bi radilo samo jednom.
        upSlot(idx,function(s){return Object.assign({},s,{status:"idle",parsed:p,paste:"",rawPaste:s.paste||s.rawPaste,
  ...
  // Drugo citanje iste poruke: radnica ne mora nista da kopira, softver koristi
  // sacuvan original. Treci pokusaj ide na DRUGI AI (Gemini) kao drugo misljenje -
  // ocitavanje je mali poziv pa je to zanemarljiv trosak.
  function ocitajPonovo(idx){
    var sl=slots[idx];
    if(!sl)return;
    var txt=String(sl.rawPaste||sl.paste||"").trim();
    if(!txt){toast2("Originalna poruka nije sacuvana \u2014 nalepi je ponovo.");return;}
    if(sl.status==="parsing")return;
    if(!window.confirm("Ovo ponovo upisuje sva polja \u2014 tvoje ru\u010dne ispravke se gube. Nastavi?"))return;
    var n=(sl.reparse||0)+1;
    upSlot(idx,function(x){return Object.assign({},x,{reparse:n});});
    var prov=n>=2?"gemini":undefined;
    if(prov)toast2("\u2728 Tre\u0107i poku\u0161aj \u2014 \u0161aljem drugom AI-u za drugo mi\u0161ljenje.");
    doParse(idx,prov,txt,true);
  }
  ...
  function SlotView(props){
    ...
    return React.createElement("div",null,
      ...
      // PARSED OK
      isMess&&s.parsed&&React.createElement("div",{style:{marginBottom:"9px",padding:"7px 11px",background:"rgba(96,176,96,.08)",border:"1px solid rgba(96,176,96,.25)",borderRadius:"7px"}},
        React.createElement("div",{style:{display:"flex",alignItems:"center",gap:"8px",flexWrap:"wrap"}},
          React.createElement("span",{style:{color:"var(--grn)"}},"✓"),
          React.createElement("span",{style:{fontSize:"11.5px"}},"Prepoznato. Proveri i ispravi."),
          // OCITAJ PONOVO (Marko 15.9.): staro dugme "Ponovi" je brisalo sve i vracalo
          // PRAZAN okvir, pa je radnica morala ponovo da kopira celu poruku iz
          // Messengera - zato ga niko nije koristio kad datum deteta nije dobar.
          React.createElement("button",{className:"btn bgd bsm",style:{marginLeft:"auto"},disabled:s.status==="parsing",onClick:function(){ocitajPonovo(idx);}},
            s.status==="parsing"?"Čita...":"↻ Očitaj ponovo"),
          React.createElement("button",{className:"btn bol bsm",onClick:function(){upSlot(idx,function(sl){return Object.assign({},sl,{parsed:null,paste:"",rawPaste:"",reparse:0,showRaw:false});});}},"Nova poruka")
        ),
        // Originalna poruka na dohvat ruke - da radnica proveri datume bez
        // prebacivanja u Messenger.
        String(s.rawPaste||"").trim()&&React.createElement("div",{style:{marginTop:"6px"}},
          React.createElement("button",{className:"btn bol bsm",onClick:function(){upSlot(idx,function(sl){return Object.assign({},sl,{showRaw:!sl.showRaw});});}},
            (s.showRaw?"▾ Sakrij":"▸ Prikaži")+" originalnu poruku"),
          s.showRaw&&React.createElement("div",{style:{marginTop:"6px",padding:"8px 10px",background:"rgba(0,0,0,.18)",border:"1px solid var(--bd)",borderRadius:"6px",fontSize:"11.5px",lineHeight:1.5,whiteSpace:"pre-wrap",maxHeight:"40vh",overflowY:"auto"}},s.rawPaste)
        )
      ),

...
async function parseMsg(text){
  ...
  for(var attempt=0;attempt<MAX_RETRIES;attempt++){
    try{
      var r=await fetch("https://astrobalkan-backend.onrender.com/api/parse",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-5-20250929",max_tokens:1500,system:systemPrompt,messages:[{role:"user",content:"Izvuci podatke iz sledece poruke:\n\n"+text}]})});
      d=await r.json();
...
export default function App(){
  ...
  async function doGen(idx){
    ...
    var hasLjubav=sl.types.indexOf("ljubav")>=0;
    var hasPosao=sl.types.indexOf("posao")>=0;
    var hasGodisnja=sl.types.indexOf("godisnja")>=0;
    var hasTranziti=sl.types.indexOf("tranziti")>=0;
    var sys;
    if(isSinastrija){
      var partnerName=sl.partner.ime||"partnera";
      // Build synastry sections dynamically
      var sSections=["1. Intro about the couple - what you immediately see (1 paragraph, 3-4 sentences)","2. Emotional compatibility - how they feel each other (1 paragraph, 4-5 sentences)","3. Love and attraction - chemistry, desire, passion (1 paragraph, 4-5 sentences)","4. Communication - how they talk and understand (1 paragraph, 3-4 sentences)","5. Challenges - karma, what they must learn (1 paragraph, 4-5 sentences)","6. 12-month forecast for the relationship - specific periods and events (1-2 paragraphs, 5-6 sentences)"];
      var sIdx=7;
      if(hasPosao){sSections.push(sIdx+". Work/business compatibility - can they work together, business partnership potential, money dynamics in the couple (1 paragraph, 4-5 sentences)");sIdx++;}
      if(hasGodisnja){sSections.push(sIdx+". Annual forecast for the relationship - each season (spring, summer, fall, winter) with concrete events (1 paragraph, 5-6 sentences)");sIdx++;}
      if(hasTranziti){sSections.push(sIdx+". Current transits affecting the relationship in next 3-6 months (1 paragraph, 4-5 sentences)");sIdx++;}
      sSections.push(sIdx+". Honest conclusion - long-term potential (1 paragraph, 3-4 sentences)");
      var focusList=[];
      if(hasLjubav)focusList.push("LJUBAV");
      if(hasPosao)focusList.push("POSAO");
      if(hasGodisnja)focusList.push("GODISNJA PROGNOZA");
      if(hasTranziti)focusList.push("TRANZITI");
      var focusStr=focusList.length>0?"\n\nCRITICAL: Client explicitly selected these additional focus areas: "+focusList.join(", ")+". Each MUST have its dedicated detailed section. Do NOT skip any.":"";
      sys="WRITE IN ENGLISH. Text will be translated to Serbian later.\n\nYou are "+aName+", a top female astrologer with 30 years of experience specializing in synastry. Feminine voice (I saw, I noticed).\n\nTASK: SYNASTRY analysis comparing TWO natal charts. Write about the RELATIONSHIP DYNAMIC between "+(sl.client.ime||"client")+" and "+partnerName+". NOT two individual analyses - this is about the COUPLE."+focusStr+"\n\nTONE:\nStart with client name in Serbian vocative and dive directly into analysis. Example: 'Zorice, u vasoj sinastriji sa "+partnerName+" vidim...' NEVER use 'sedi nasuprot mene', 'pogledaj me', 'sit across from me', 'imagine sitting' or any physical meeting metaphors.\n\nVOCATIVE: Zorica=Zorice, Milica=Milice, Dragana=Dragane, Ivana=Ivana, Jelena=Jelena, Ana=Ana, Jovana=Jovana, Marko=Marko, Ivan=Ivane, Nikola=Nikola, Darko=Darko.\n\nFORBIDDEN:\n- NEVER write planet names, houses, degrees, astrological terms. Write what it MEANS for their relationship.\n- NEVER 'sedi nasuprot mene' or physical metaphors.\n\nSTRUCTURE (ALL sections below are REQUIRED, every section must be present):\n"+sSections.join("\n")+"\n\nLENGTH: 1200-1800 words total. Complete ALL sections - be concise but do not skip.\n\nAT THE END write exactly:\nHvala ti puno na poverenju i zelim ti odnos ispunjen ljubavlju, razumevanjem i radoscu.\n\nAstrolog "+aName+" \u2764\uFE0F\n\nToday is: "+todayStr;
    }else{
      // Build individual analysis sections dynamically based on what user checked
      var iSections=["1. Emotional intro - who is this person (1 paragraph, 3-4 sentences)","2. Inner world - character, emotions, psychology (1-2 paragraphs, 6-8 sentences total)"];
      var iIdx=3;
      if(hasLjubav){iSections.push(iIdx+". LJUBAV I PARTNERSTVO - MAJOR SECTION - current love situation, who is their ideal partner, concrete events in love next 12 months, romantic periods, warnings (2 paragraphs, 8-10 sentences)");iIdx++;}
      if(hasPosao){iSections.push(iIdx+". POSAO I NOVAC - MAJOR SECTION - current career situation, career direction, specific business opportunities, money forecast next 12 months, best periods for work/investment, financial warnings (2 paragraphs, 8-10 sentences)");iIdx++;}
      if(hasGodisnja){iSections.push(iIdx+". GODISNJA PROGNOZA - 12-month forecast divided by specific periods (Apr-Jun, Jul-Oct, Nov-Feb, Mar-Apr) with concrete events per period (2 paragraphs, 8-10 sentences)");iIdx++;}
      if(hasTranziti){iSections.push(iIdx+". TRANZITI - current planetary transits and their impact in next 3-6 months, which areas of life are activated (1-2 paragraphs, 5-7 sentences)");iIdx++;}
      iSections.push(iIdx+". Health - brief but concrete (1 short paragraph, 2-3 sentences)");iIdx++;
      iSections.push(iIdx+". Honest conclusion - truth about their life path (1 paragraph, 3-4 sentences)");
      var iFocus=[];
      if(hasLjubav)iFocus.push("LJUBAV");
      if(hasPosao)iFocus.push("POSAO");
      if(hasGodisnja)iFocus.push("GODISNJA PROGNOZA");
      if(hasTranziti)iFocus.push("TRANZITI");
      var iFocusStr=iFocus.length>0?"\n\nCRITICAL FOCUS AREAS: Client explicitly selected: "+iFocus.join(", ")+". These are the PRIMARY themes of this analysis. Each selected area MUST have its dedicated detailed section. Do NOT skip ANY selected area. Each selected area should be a MAJOR part of the analysis, not a brief mention.":"";
      sys="WRITE IN ENGLISH. Text will be translated to Serbian later.\n\nYou are "+aName+", a top female astrologer with 30 years of experience. Feminine voice (I saw, I noticed)."+iFocusStr+"\n\nTHIRD PERSON RULE:\nIf Name contains family relation (daughter, son, brother, sister, husband, wife, mom, dad, friend, aunt, uncle) - analysis is about that person, speak TO client ABOUT them. Third person: 'Your daughter is a woman who...', 'He is a man who...'. If real name (Milica, Marko) - address directly.\n\nTONE:\nStart with name in Serbian vocative and dive directly into analysis. VOCATIVE: Zorica=Zorice, Milica=Milice, Dragana=Dragane, Ivana=Ivana, Jelena=Jelena, Ana=Ana, Jovana=Jovana, Marko=Marko, Ivan=Ivane, Nikola=Nikola. Example: 'Zorice, u tvojoj karti vidim...' NEVER 'sedi nasuprot mene', 'pogledaj me', 'sit across from me', 'imagine sitting' or any physical metaphors.\n\nFORBIDDEN:\n- NEVER planet names, houses, degrees, astrological terms. Write what it MEANS concretely.\n- NEVER physical meeting metaphors.\n- NEVER list zodiac sign traits.\n- NEVER skip a section user explicitly selected.\n\nSTRUCTURE (ALL sections below are REQUIRED, every section must be present):\n"+iSections.join("\n")+"\n\nLENGTH: 1200-1800 words total. Include EVERY section - be concise but do not skip.\n\nAT THE END write exactly:\nHvala ti puno na poverenju i zelim ti zivot ispunjen mirom, radoscu i srecom.\n\nAstrolog "+aName+" \u2764\uFE0F\n\nToday is: "+todayStr;
    }

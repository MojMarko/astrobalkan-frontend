...
export default function App(){
        ...
        if(partnerMissing){
          ...
          if(isFamily){
            ...
            if(partnerIme){
              ...
              p.imaPartnera=true;
              p.partner=Object.assign({ime:partnerIme,datum:partnerDatum,vreme:"",mesto:"",zemlja:""},p.partner||{});
  ...
  async function doCalc(idx){
    ...
    try{
      // FINAL DEFENSE: ako imamo originalni paste, re-ekstraktuj vreme za klijenta i partnera.
      // Ovo prepravlja stari kesirani vreme iz localStorage-a (pre fix-a za AM/PM).
      var raw=sl.rawPaste||"";
      var rawTimes=extractTimesFromPaste(raw);
      console.log("[doCalc] rawPaste len="+raw.length+" extracted times:",rawTimes,"current client.vreme="+sl.client.vreme+" partner.vreme="+(sl.partner&&sl.partner.vreme));
      if(rawTimes[0]&&rawTimes[0]!==sl.client.vreme){
        console.log("[doCalc] OVERRIDE client.vreme: "+sl.client.vreme+" → "+rawTimes[0]);
        upSlot(idx,function(s){return Object.assign({},s,{client:Object.assign({},s.client,{vreme:rawTimes[0]})});});
      }
      if(sl.hasPart&&rawTimes[1]&&rawTimes[1]!==sl.partner.vreme){
        console.log("[doCalc] OVERRIDE partner.vreme: "+sl.partner.vreme+" → "+rawTimes[1]);
        upSlot(idx,function(s){return Object.assign({},s,{partner:Object.assign({},s.partner,{vreme:rawTimes[1]})});});
      }
      var slCur=slots[idx];
      var fixedClient=rawTimes[0]?Object.assign({},sl.client,{vreme:rawTimes[0]}):sl.client;
      var fixedPartner=(sl.hasPart&&rawTimes[1])?Object.assign({},sl.partner,{vreme:rawTimes[1]}):sl.partner;

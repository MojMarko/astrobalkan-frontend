...
function fmtText(text){
  ...
  var raw=t.split(/(?<=[.!?])\s+/);
  // Merge fragments that start with conjunctions back into previous sentence
  var sents=[];
  for(var i=0;i<raw.length;i++){
    if(sents.length>0&&/^(ili |a |te |pa |i |ni |niti |kao |vec |nego |tj\.|npr\.|tj |npr )/i.test(raw[i])){sents[sents.length-1]+=" "+raw[i];}
    else{sents.push(raw[i]);}
  }
  var out=[];
